...
function manejarReserva(event) {
  ...
  if (!fecha || isNaN(comensales) || comensales < 1 || comensales > 50) {
    alert('Por favor, ingresa una fecha válida y una cantidad de comensales entre 1 y 50.');
    return;
  }
  ...
  const totalComensalesReservados = reservasPorFecha.reduce((total, reserva) => total + reserva.cantidad, 0);

  // Verificar si hay capacidad suficiente
  if (totalComensalesReservados + comensales > 50) {
    alert('No es posible realizar la reserva. Se ha alcanzado la capacidad máxima para esa fecha.');
    return;
  }
  ...
  console.log('Historial completo de reservas:', JSON.parse(localStorage.getItem('reservas')));
